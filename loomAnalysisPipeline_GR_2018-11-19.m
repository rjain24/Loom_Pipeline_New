...
% Sept 2018- to be adapted for analysing loom-induced escapes with Roshan
% Jain
...
%12 average angular velocity in deg/sec
...
for mm = 1:size(locsz,1)
    ...
    tmp_swim_bouts(mm,13) = (locsz(mm)-idx_tap(mm))*frame_length_calc_ms; %if there is an error --> error in tap/escape ID
        % idx_tap is too small!  This should be using loomstart, but it doesn't 
        % match locsz (all bouts) in size [RJ 11-19-18]
    
    %peak angular velocity [10/31/18 RJ This is not calculating correctly - locsz is a vector, need to rethink]
    %tmp_swim_bouts(mm,17) = nansum(abs(tmp_ang_vel(locsz(mm,1):locsz(mm,2))))/((locsz(mm,2)-locsz(mm,1))*1.33)/1000; %in deg/sec already
    % [RJ] I want to sum the 
